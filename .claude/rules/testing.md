...
| **文書が保証として書いていたのに動いていない** | 9 | **繋いだ**（`shared/team.ts` の作業一式） |
...
### 日本語の校正を機械で見るか（2026-09-14 に測った）

textlint の `preset-ja-technical-writing` を日本語の文書 30 本に当てて **220 件**。内訳は
一文が長い 102、同じ助詞の連続 56、である／ですますの混在 30、文末の句点 17、その他 15。
**約 84% がこの repo の書き方に対する誤検出**だった —— 行を折るので 1 文が繋がって数えられ、
コードの羅列（`system:init / system:hook_started / …`）が 194 字の文になり、`…より:` の行が
「句点が無い」になり、Forgejo の PR 番号 `!1` が「感嘆符」になる。

表記ゆれを自分で数え直すと、**本物は 2 組だけ**だった（`ユーザ`/`ユーザー` 2 件、`一つ`/`1 つ` 3 件）。
画面の文言 205 件にはゆれが無く、40 字を超えるものも 1 件だけ。2026-09-11 に人が 188 件を洗ったのが効いている。

**入れないもの**: textlint 本体（誤検出を抑える設定の保守が、得られるものより高い）、
校正に特化した LLM（呼ぶたびに答えが揺れ、網と費用が要り、門にならない。規則 9）。
**そもそも、この週に利用者が指摘した 4 つ（盤面・出す・いま・頼む）は、どちらも素通しする** ——
文法も表記も正しく、間違っていたのは語が何を指すかで、校正の道具が見る層より上にある。
...
| 表記のゆれ | `test/copy.test.ts` | 日本語の文書 17 本と画面の文言で、`ユーザ`（長音なし）と `一つ` が出る。**測ってから足した 2 組だけ** |
...
単体で 8 回回すと全部通り、85 個の worker が走る verify の中でだけ落ちる。原因は
`await new Promise((r) => setTimeout(r, 30))` —— **非同期の鎖が 30ms で終わる前提**を置いていた。

条件が満たされるまで待ち、上限で諦める形に直した（`until(() => s.sent.length >= 2)`）。
待つ理由が「時間」ではなく「何が起きるか」になるので、**遅い機械でも速い機械でも同じことを見る。**
時間で待ってよいのは「起きないことを確かめる」ときだけで、そこは残してある。
