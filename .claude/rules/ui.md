...
| タブの **盤面** | **作業**（2026-09-11、利用者の指摘。盤は無い。画面が出しているのは共有フォルダの作業と、その記録。「決めた語彙」として残していたが、`ブレイン`（役の名）や `共有フォルダ`（そのもの）と違い、何も指していなかった） |
| **読めなかった札** | **読めなかった作業**（同じ理由。札も比喩） |
...
残したのは「ブレイン」「実行役」「共有フォルダ」 —— team.md §12 と GOAL.md で決めた名前で、
文書と画面で同じ字にしてある。
...
利用者の判断で、**中で読むこと**と**対象を指して会話を始めること**を足した（Issue #47・#48）。
**編集はしない** —— docs/GOAL.md の「WYSIWYG エディタをやらない」は変えない。直すのはエージェントで、
指すのは人で、承認は人が持つ（規則 1）。Zed との違いは、自分でカーソルを置いて書き換える面を持たないこと。

### 読む

| 何 | どこ | 中身 |
| --- | --- | --- |
| 口 | `readFile(cwd, path)`、`main/readfile.ts` | **読む口だけ。書く口は作らない。** 上限 512KB で頭だけ返し、切ったと言う |
| 関所 | `shared/readfile.ts` の `readableUnder` | **作業ディレクトリの中だけ。** `..` で外に出る経路を閉じ、前方一致だけで判定しない（`/w` は `/work` に当たらない）。会話に出たパスをそのまま渡す経路なので、エージェントが書いたパスが混ざりうる（§26 の敵） |
| 字か | `looksText` | 頭 4KB に NUL があれば断る。画像を等幅で描くと画面が壊れる |
| 画面 | `components/FileView.tsx` | 頁（§32）と**同じ場所**に出す。どちらか一方だけ —— 両方積むと会話が押し出される。`key={path}` で作り直すので、effect の中で状態を戻さない |
| markdown | 木で描く（§25）。「字で見る」で行が出る | **木のままでは行を指せない。** 指したいときだけ字に切り替える |

### 指す

`shared/mention.ts` が文を作る。**入力欄に足すだけで送らない。**

| どこから | 文 |
| --- | --- |
| ファイルの行（「話す」） | `src/x.ts について: ` |
| 読んでいる画面で行を選ぶ | `src/x.ts:12 について: ` / `src/x.ts:12-20 について: ` |
| 差分（会話の中と、PR タブ） | `src/x.ts の差分について: ` |

- 選ぶ向きで文が変わらない（`12-20` と `20-12` は同じ）
- **打ちかけの文を捨てない。** 空なら入れるだけ、書きかけがあれば行を変えて足し、同じものは 2 度足さない
- 承認の札（`PermissionBar`）には付けない。押す場所が「許可 / 拒否」の隣で、読む場面ではない

### やらなかったこと

- hunk 単位で指すこと。ファイル単位にした —— どの版の差分かは会話の文脈にある
- 編集。`readFile` の隣に `writeFile` を作らない。作った時点で「読む道具」ではなくなる

## 35. 右パネルの 4 枚（2026-09-13）

### 何が壊れていたか

**右パネルは設計されていなかった。** 段ごとにタブを 1 枚ずつ足した結果、300px の柱に 7 枚
（情報 / ファイル / 資料 / 作業 / ループ / PR / ブランチ）が並び、性格が混ざっていた。
この節を書くまで、ui.md にタブ**全体**を論じた節は 1 つも無かった（個々のタブの節だけがあった）。

利用者の指摘（2026-09-13）が 2 件とも、この欠如の表れだった。

| 指摘 | 表に出たもの | 本当の原因 |
| --- | --- | --- |
| 「このタブ使いにくい」（ファイル） | 長いパスが読めない、釦の字が縦に折れる | **問いが違う。** 人が知りたいのは「自分のリポジトリの何が変わったか」なのに、`/private/tmp/.../scratchpad/*.py` や `~/.claude/memory/*.md` まで一覧にしていた。パスの切り方を直しても読みやすくならない |
| 「作業もよくわからない」 | 「作業 0 件 / 記録 1 行 / start …」 | **常設にする理由が無い。** 実行役を使わないセッションでは永久に空で、代わりに Izuna 自身の起動記録が出ていた |

重複もあった。worktree の状態は「情報」と「ブランチ」、push 済みかは「情報」と「PR」、
Issue は「情報」と新しいセッションの画面。同じ事実を 2 か所で出していた。

### 決めたこと

**タブは機能ではなく問いで分ける。4 枚。** 状態を見る 2 枚と、前に進める 2 枚。

| タブ | 答える問い | 中身 |
| --- | --- | --- |
| **Status** | どこで、何が動いていて、あと何回頼めるか | worktree と枝、実行役（走っているもの・重なり・壊れた札）、上限。共有フォルダの記録は畳んで置く |
| **Changes** | **自分のリポジトリの**何が変わったか | worktree の中のファイル（書いた順）。外のものは「外のファイル n 件」に畳む |
| **PR** | 次にどこへ出すか、何を片付けるか | Sandbox → Upstream の二段（`Forge`）と、worktree の片付け（`Worktrees`） |
| **Run** | 人が始める操作 | 資料の skill（`Docs`）、自律ループと起床の予約（`Loop`） |

### 規律

- **タブは消えない。** 中身が無いときは節ごと畳む。タブが出たり消えたりすると、押す場所が動いて探し直しになる
- **空の節は出さない。** 「作業 0 件」のような見出しだけの節を残さない。何も無いときは、何が起きたらここに出るかを 1 行で言う
- **節には必ず名前を付ける**（2026-09-14。利用者の「セクションで分かれていないから何か分からない」）。
  `Run` に資料と自律ループと予約を積んだとき、見出しが無かったので、どこからどこまでが何なのか読めなかった。
  見出しは `ui.tsx` の `Section` 1 つに寄せた（同じ形が `Board.tsx` にもあった）
- **幅はタブで変えない**（同日）。`Status` だけ 288、他が 360 だったので、押すたびに柱が動いていた。
  全部 360 —— 読んでいた場所を見失わせない
- **押した結果が、押した場所から見えること**（2026-09-14。利用者の「頼むを押しても何もフィードバックが
  出ず、ちゃんとできているか分からない」）。**効き目が別の柱に出る操作**が 4 つあった ——
  `Run` の「頼む」（会話に送る）と、`Changes` / ファイル / 差分の「話す」（入力欄に入れる）。
  どれも今週足したもので、押しても押した場所は何も変わらなかった。直し方は 2 通り。

  | 効き目 | どうするか |
  | --- | --- |
  | 送った（戻せない） | その場に結果を出す（`ui.tsx` の `Result`）。`Forge` と `Worktrees` が既にそうしている |
  | 入力欄に入れた（まだ送っていない） | **入力欄へ焦点を移す。** 入れたことが見え、そのまま続けて打てる |

  釦を足すときは、押した人の目に何が変わるかを先に決める。変わるものが無いなら、その釦は要らない
- **中身の無いものに札を出さない**（2026-09-14。利用者の「思考がこうなっているけど」）。
  要約の返らない思考ブロックに「思考 ▸」を出していた。開いても「（要約は返っていません）」が出るだけで、
  **押した場所に何も起きない**（上の規律に反する）。畳む札は、開いたら何か出るときだけ置く。
  **「無い」と書いた文字を中身の代わりに置かない** —— それは中身ではなく、Izuna の言い訳である
- **同じ事実を 2 か所に出さない**（2026-09-14 に当てた。順番の 5）。1 つの事実に 1 つの置き場を決める。

  | 事実 | 置き場 | 落とした先 |
  | --- | --- | --- |
  | worktree と枝 | `Status`（`Inspector`） | —— |
  | sandbox に push 済みか、PR を作る釦 | `PR`（`Forge`） | `Inspector` から消した |
  | open な Issue | 新しいセッションの画面 | `Inspector` から消した。Issue は「次に何を始めるか」で、`Status` の問い（どこで・何が動いて・あと何回）ではない |
  | 走っている実行役とその出力 | 会話の柱（`TaskPanel`） | `Status` には重ねない。あちらに残るのは共有フォルダ由来（`Board`）だけ |
  | ツールの名前 | 会話の札の左 | `describeToolInput` は言うことが無ければ**空を返す**。名前を返していたので `AskUserQuestion AskUserQuestion` と 2 度出ていた（2026-09-14。人に問うツールは問いそのものを一言にする） |

  **出さないものは読みにも行かない。** `Inspector` は `remotes` / `isPushed` / `ghIssues` / `forgeIssues` の
  4 つを呼ばなくなった（`test/renderer/inspector.test.tsx` が、呼んだら落ちる形で見張る）
- **状態と操作を混ぜない。** `Status` と `Changes` は読むだけ。押して何かが起きるのは `PR` と `Run`
- **タブ名は比喩にせず、英語のままにする**（利用者の指摘、2026-09-13。2 段階で決まった）。
  最初の案「いま / 変更 / 出す / 頼む」は、「出す」が行き先を言わず、「いま」が時制、「頼む」が動作だった。
  名詞に直して「状態 / 変更 / PR / 依頼」にしたが、**訳したこと自体が間違い**だった ——
  §17.4 は既に「名前は訳さない」と決めていて、`plan` / `acceptEdits` はそのまま出している。
  `git status` / `diff` / `worktree` / `PR` を日常的に読む人に、訳して出す理由が無い。
  **タブ名だけ英語、本文は日本語。** `Run` は「ここで始めるものを置く」の意（skill・ループ・予約が
  どれも「走らせるもの」で一貫する）

### 「変更」は worktree の中だけ（利用者の判断、2026-09-13）

エージェントは作業ディレクトリの外も触る（scratchpad、`~/.claude/`、一時ディレクトリ）。
**それは「変更」の答えではない。** 外のものは数だけ出して畳み、開けば見える —— 捨てはしない
（黙って消すと、触ったことに気づけない）。

`git status` を正にする案は取らなかった。会話から導く形（`shared/touched.ts`）は**保存層を持たない**
という規則 2 と同じ根で、git に聞くと「誰が触ったか」が落ちる。**未検証**: 会話と `git status` が
食い違う場面（人が手で直したとき）。そのときは「いま」の worktree の数字と突き合わせる。
...
| --- | --- |
| 情報（`Inspector`） | **Status** |
| ファイル（`Files`） | **Changes**（worktree の中だけに絞る） |
| 作業（`Board`） | **Status**（実行役が動いているときだけ。記録は畳む） |
| PR（`Forge`） | **PR** |
| ブランチ（`Worktrees`） | **PR**（`Forge` の下） |
| 資料（`Docs`） | **Run** |
| ループ（`Loop`） | **Run**（`Docs` の下） |

### 空の節を畳む（2026-09-14。順番の 4）

`log.md` を「中身がある」に数えない。実行役を使わないセッションで共有フォルダに入っているのは
**Izuna 自身が書いた起動の 1 行だけ**で、それを数えると「作業 0 件 / 記録 1 行 / start …」という
見出しだけの節が常に出る（利用者の「作業もよくわからない」の正体）。

| 状態 | 出すもの |
| --- | --- |
| 共有フォルダが無い | 「共有フォルダがありません」 |
| **作業・着手できるもの・要約・決定・重なり・壊れた札がどれも 0** | 節を 1 つだけ。「ブレインが共有フォルダに作業を書くと、ここに出ます」 |
| どれかがある | 今までどおり。**記録だけは畳む**（行数は見えるが、開くまで中身は出ない） |

### 枠の窓は、返ってきたものを全部出す（2026-09-14）

利用者が `/usage` と Izuna を並べて気づいた。

| | `/usage` | Izuna |
| --- | --- | --- |
| 5 時間の窓 | 現在のセッション **1%** | 5時間 **21%** |
| 週の窓 | 今週 **63%** | 7日 **62%** |
| **今週の Fable** | **100%**（尽きている） | **出ていない** |

出どころは同じ（`rate_limit_event` の `rate_limit_info.unifiedWindows`）で、原因は 2 つだった。

1. **古びる。** `rate_limit_event` はターンが走っているあいだしか来ない。窓が空いたあと
   何も送っていないセッションは、**空く前の数字を出し続ける**（21% は前の窓のもの）。
   取り直す口は無く、`transcript.limits` はセッションごとに持っている。
   いまは `resetsAt` を過ぎていたら「前の窓のもの」と言う
2. **2 つに決め打っていた。** `five_hour` と `seven_day` だけを取り出し、
   **それ以外は黙って捨てていた。** 上流が窓を増やすと落ちる形で、実際に落ちていたのが
   **100% の窓** —— つまり、そのとき一番効いている制約だけが見えていなかった

直したこと。`limits` は窓の一覧になり、鍵ごとに重ねる（片方しか来ない回があっても落とさない）。
**名前は訳さない**（§17.4）ので、知らない鍵は上流の字のまま出す。`five_hour` を「現在のセッション」と
訳さないのは、Izuna の画面に既に「セッション」（左の一覧の 1 件）があって衝突するからで、
代わりに空く時刻を隣に置いて `/usage` と突き合わせられるようにした。
帯に出す 1 つは**一番詰まっている窓**にした —— 5 時間だけを出していたので、週が尽きていても
余裕があるように見えた。100% は赤にする（7 割の琥珀と同じ色では読み落とす）。

### 順番（全部終わった。2026-09-14）

1. ~~この節（設計）~~
2. ~~タブを 4 枚にし、いまの部品をそのまま差し込む（骨）~~
3. ~~`Changes` を worktree の中だけに絞る~~
4. ~~`Status` の空の節を畳む~~（上）
5. ~~重複を消す~~（規律の表）

**骨を先に入れる。** 中身を先に直すと、どのタブに置くかが決まらないまま形が動く。

### 窓は最大化して開く（2026-09-14）

1280×860 で開いていた（electron-vite の雛形のまま）。利用者の「ちょっと小さい感じで開かれる」から。
この画面は柱が 3 本（セッションの一覧・会話・右パネル）あり、**狭いと会話の柱が先に潰れる** ——
両脇は幅が決まっているので、削られるのは真ん中だけである。

`ready-to-show` で `maximize()` してから `show()` する。順番を逆にすると、
開いたあとに大きくなる動きが見える。`width` / `height` は残してある ——
最大化を解いたときに戻る大きさになる。

## 37. 配色を選ぶ（2026-09-14）

利用者の求めから。§21 で「利用者の Ghostty に合わせる」を入れたが、**合わせる先を選べなかった** ——
Ghostty を使っていない人には既定しか無く、Ghostty を使っている人も別の配色を試せなかった。

### 決めたこと

**作る仕組みは増やさない。** どの選び方も、最後は `GhosttyColors` を 1 つ作って `skinFrom()` に渡すだけ
（段の作り方・コントラストの床は §21 のまま）。増えたのは**どこから色を取るか**の分岐だけである。

| 選び方 | 色の出どころ |
| --- | --- |
| Ghostty に合わせる（既定） | `~/.config/ghostty/config` と、そこが指すテーマ（§21 の読む順） |
| Ghostty のテーマから選ぶ | 実機にあるテーマファイル 1 本。利用者の置き場が先、同梱が後 |
| Izuna の既定 | 無し（`null` を返す。既定の色で出る） |
| 自分で決める | 画面で選んだ 5 色 |

**書体と行送りは、どの選び方でも利用者の Ghostty から借りる。** 配色だけを差し替える ——
テーマを選んだからといって、書体の好み（`font-thicken`、UD 書体。§21）まで捨てる理由が無い。

### 自分で決めるのは 5 色だけ

`skinFrom()` が読む番号だけを開ける（地・文字・palette 11 / 14 / 9）。16 色を全部開けても、
Izuna が使うのはこの 5 つで、残りは触っても何も変わらない ——
**変わらないものを触らせない**（§17.4「事実でない説明を添えない」と同じ根）。

| 画面の字 | 何の色か |
| --- | --- |
| 地 / 文字 | `background` / `foreground` |
| 人の判断待ち | palette 11（アンバーの規律。§17） |
| 済んだこと | palette 14 |
| 壊れたこと | palette 9 |

読む字のコントラスト（AA の床。§21）は Izuna が保つので、**読めない組み合わせを選んでも字は読める。**

### 設定は準備と別にする

準備（`ForgeSetup`）は「使い始めるために要るもの」で、無いと動かない。設定は「動くけれど好みで
変えるもの」である。同じ覆いに積むと、**初めて開いた人がどれをやらないと始まらないのかを読めなくなる。**

- 選んだ瞬間に当てて、設定にも書く（`main/config.ts` の `saveConfigValue`）。
  **押した結果はその場に見える**（§35）—— 色が変わり、`Result` が何を選んだかを言う
- 設定ファイルは**鍵 1 つを重ねて書き戻す。** 既定を全部書き出すと、あとで Izuna の既定が変わっても
  古い値が残り、なぜ変わらないのか分からなくなる。**読めないファイルは上書きしない**
- 生の `<input>` / `<select>` は書かない（§17）。`ui.tsx` の `ColorInput` と `Select` に寄せた ——
  `test/design-system.test.ts` が、書いたその場で落とした

### やっていない

- 開いているターミナルの中の配色。`TerminalPane` は mount のときに読むので、開き直すまで前のまま
  （画面にそう書いてある）
- 明るいテーマでの見え方の確認。`skinFrom` は明るい地でも通るように書いてあるが（§21）、
  **撮って測ってはいない**
