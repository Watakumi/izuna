...
| タブ | 答える問い | 中身 |
| --- | --- | --- |
| **状態** | どこで、何が動いていて、あと何回頼めるか | worktree と枝、実行役（走っているもの・重なり・壊れた札）、上限。共有フォルダの記録は畳んで置く |
| **変更** | **自分のリポジトリの**何が変わったか | worktree の中のファイル（書いた順）。外のものは「外のファイル n 件」に畳む |
| **PR** | 次にどこへ出すか、何を片付けるか | Sandbox → Upstream の二段（`Forge`）と、worktree の片付け（`Worktrees`） |
| **依頼** | 人が始める操作 | 資料の skill（`Docs`）、自律ループと起床の予約（`Loop`） |
...
- **状態と操作を混ぜない。** 「状態」と「変更」は読むだけ。押して何かが起きるのは「PR」と「依頼」
- **タブ名を比喩にしない**（利用者の指摘、2026-09-13）。最初の案は「いま / 変更 / 出す / 頼む」だったが、
  「出す」は行き先を言っておらず、「いま」は時制、「頼む」は動作だった。§17.4 のとおり**名詞**にし、
  既にある語彙から採る —— `PR` は訳さない名前（§17.4）、`依頼` は入力欄が既にそう呼んでいる
  （「依頼を書く」「最初の依頼」）
...
| --- | --- |
| 情報（`Inspector`） | **状態** |
| ファイル（`Files`） | **変更**（worktree の中だけに絞る） |
| 作業（`Board`） | **状態**（実行役が動いているときだけ。記録は畳む） |
| PR（`Forge`） | **PR** |
