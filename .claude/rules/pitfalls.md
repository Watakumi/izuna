...
- **macOS の Docker では、127.0.0.1 に束ねたホストのサービスにコンテナから届く**（2026-09-10 に測った）。
  ...
  （docs/ACTIONS.md § 壁 2）。
- **Electron を入れ替えた直後の初回起動だけ、main が約 5 秒止まる**（2026-09-15、39.8.10 → 41.10.7 で踏んだ）。
  症状は `pnpm e2e` が `ghosttySkin()` の次（**最初に外の道具を呼ぶ口**）から戻らないこと。
  15 分待っても戻らず、Electron 41 の退行だと読んだが、**違った**。

  | 測ったもの | 39.8.10（node 22.22.1） | 41.10.7（node 24.18.0）入れ替え直後 | 同・数回起動したあと |
  | --- | --- | --- | --- |
  | アプリ内で最初の `run()` | 198–209 ms | 4,798–7,440 ms | 210 ms |
  | 2 回目以降の `run()` | 5 ms | 6 ms | 5 ms |
  | `readLoginShellEnv()` 単体 | 121 ms | 117–290 ms | 117 ms |
  | main のイベントループ | 止まらない | **4,787 ms 止まる**（起動から約 6 秒） | 止まらない |

  **遅いのはログインシェルではない。** `loginShellEnv()` は覚えたあとなのに、その
  `await` に 4,571 ms かかっていた —— つまり**イベントループごと止まっていた**。
  止まる位置は `forgeFacts` の最初の `safeStorage`（キーチェーン）読み出しの窓。

  **原因は未検証。** キーチェーンの項目 `izuna Safe Storage` は 39 のバイナリが作ったもので、
  41 は `.pnpm/electron@41.10.7_…/dist/Electron.app` という**別の署名のバイナリ**から触る。
  macOS が初回に認可の経路へ入るなら辻褄は合うが、**認可のダイアログは見ていない**。
  分かっているのは「入れ替えた直後の数回だけで、そのあと消え、戻らない」ことだけ。

  対処は待つこと。**版を上げた直後の 1 回目の `pnpm e2e` を、退行と読まないこと。**
  読む前に 2 回目を走らせる。`scripts/lib/electron.ts` の `call()` には上限が無いので、
  止まったように見えるが、実際には返ってくる。
