  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: S.lg }}>
      {questions.map((q) => {
        ...
        return (
          <div key={q.question} style={{ display: 'flex', flexDirection: 'column', gap: S.sm }}>
            {q.header && (
              <span style={{ font: `${F.micro}px ${MONO}`, color: C.dim2 }}>{q.header}</span>
            )}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: S.xs }}>
              {q.options.map((o) =>
                q.multiSelect ? (
                  <Check
                    key={o.label}
                    checked={cur.includes(o.label)}
                    onChange={() => toggle(q, o.label)}
                  >
                    <span>{o.label}</span>
                    {o.description && (
                      <span style={{ color: C.dim2, marginLeft: S.sm }}>{o.description}</span>
                    )}
                  </Check>
                ) : (
                  <button
                    ...
                    style={{
                      ...
                      background: cur.includes(o.label) ? C.raised : 'transparent',
                      color: C.ink2,
                      fontSize: F.body
                    }}
                  >
      ...
      <div style={{ display: 'flex', gap: S.sm, justifyContent: 'flex-end' }}>
        ...
        <Button kind="primary" disabled={!ready} onClick={() => onAnswer(answers())}>
          答える
        </Button>
      </div>
    </div>
