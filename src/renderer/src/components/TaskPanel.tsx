...
function One({ task }: { task: TaskRun }): React.JSX.Element {
  ...
  return (
    <div style={{ border: `1px solid ${C.line2}`, borderRadius: 9, background: C.surface, overflow: 'hidden' }}>
      <div onClick={() => setOpen((v) => !v)}
        ...
        <span style={{ fontSize: 12.5, fontWeight: 500, flexShrink: 0 }}>
      ...
      {open && (
        <div style={{ padding: '0 13px 13px', display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          {texts.map((b, i) => (
            <div key={i} style={{ color: C.ink2, fontSize: 12.5, lineHeight: 1.75, whiteSpace: 'pre-wrap' }}>
              {b.text}
            </div>
          ))}
...
export function TaskPanel({ tasks }: { tasks: TaskRun[] }): React.JSX.Element | null {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 9, padding: '0 24px 4px' }}>
