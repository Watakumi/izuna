...
`test/design-system.test.ts` がスケール外の値・部品の重複定義・生の色を
検出して落とす。**「とりあえず許可リストに足す」をしない。**
落ちたら値を直すか、スケール自体を見直す。

`padding` は検査していない（52 通りあったが、多くは `ui.tsx` の部品が
吸収した）。残りは個別の余白で、スケール化の効果より制約の窮屈さが勝つ。
