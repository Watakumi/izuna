...
| 3 | **CLAUDE.md を分ける** | 「先に読む重要規則」だけ CLAUDE.md に残し、残りは `.claude/rules/*.md` に `globs` 付き。触るパスのときだけ読まれる。理由は `rules/token-discipline.md` | Izuna の CLAUDE.md は 2,191 行で毎セッション全部読まれる。§7 や §21 の罠は、そのファイルを触るときにだけ要る |
