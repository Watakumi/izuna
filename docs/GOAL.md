...
**自分の Forgejo を持っている人**（2026-09-09 に「作者ひとり」から改めた）。
コードを他人のサーバに置きたくないが、エージェントによる並列開発の効率は欲しい人。
Forgejo が無い人は対象にしない —— sandbox の分離が唯一の差別化で、それを外すと
既存の GUI と同じものになる。

汎用製品として競合に勝つことは目標にしない。機能の多さで Nimbalyst に
追いつく必要も無い。**その運用に噛み合うこと**が評価軸である。
配るからには、**他人の環境で準備が通ること**が要る（docs/SETUP.md）。
...
1 つのリポジトリに対し、複数の Claude Code セッションを
...
共有フォルダ `~/.izuna/teams/<team>/` を 1 つ決め、`--add-dir` で全員に渡す。
...
並列で一番壊れる箇所を無人にすることになる。「人が持つ」の中身は**権限モードを人が選ぶ**こと ——
`default` / `acceptEdits` なら実行役の分も 1 件ずつ人に来る。`auto` を人が選べば Claude Code の
分類器が引き受け、人には来ない（2026-09-10 に実測。`.claude/rules/team.md` §12）。
Izuna が作らないのは、ブレインという別のエージェントに実行役の要求を許可させる形である。
...
- **拡張機構・プラグイン SDK**
- **自動更新**（配布はする。更新の経路は供給網の一番弱い所になるので、署名の検証まで見てから）
