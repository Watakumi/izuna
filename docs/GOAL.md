...
| 1 | 会話 UI | ✅ 済 |
| 2 | `/` パレット | ✅ 済 |
| 3 | worktree 並列 | ✅ 済（人が起こす分。ブレイン配下は agent isolation） |
| 4 | ブレインと実行役 | ✅ 済（Agent ツールのサブエージェント。承認は人間に届く） |
| 5 | 二段の PR | ✅ 済（Forgejo クライアントと `gh`。Sandbox → Upstream） |
| 6 | ターミナル | ✅ 済（ghostty-web + node-pty。worktree のシェルに降りられる） |
...
画面のモックは持たない（CLAUDE.md §16）。**実装が正。**
決めたことと、まだ作っていない画面だけをキャンバスに残す。

https://claude.ai/code/artifact/873094d6-cdf6-46b4-b488-a69ab9e3641e
