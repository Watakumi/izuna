...
const tool = (
  name: string,
  path: string,
  state: 'done' | 'denied' = 'done'
): Transcript['items'][number] => ({
  kind: 'assistant',
  id: 'm1',
            ...
            blocks: [
              {
                ...
                id: 't',
                name: 'Write',
                input: { file_path: '/Users/x/work/izuna/c.ts' },
                state: 'done',
                result: null
              }
            ]
...
describe('盤面', () => {
  const board = (over: Partial<TeamBoard> = {}): TeamBoard => ({
    dir: '/t',
    brief: { issue: null, created: null, body: '' },
    tasks: [],
    errors: [],
    summaries: [],
    decisions: [],
  ...
  it('**重なる組と読めなかった作業を目立たせる**', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText('同時に走らせてはいけない組があります')).toBeTruthy()
    )
    const text = container.textContent ?? ''
...
describe('セッション一覧', () => {
  ...
  it('リポジトリで束ね、承認待ち・実行中・終了・待機を言葉で出す', () => {
    const panels = [
      ...
      panel(
        { id: 'b', label: 'B', cwd: '/Users/x/.izuna/worktrees/izuna/feat', branch: 'feat' },
        { running: true }
      ),
    ...
    const text = container.textContent ?? ''
    for (const w of ['承認待ち', 'feat · 実行中', '終了', '待機', 'izuna', 'other'])
