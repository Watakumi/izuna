...
const tool = (name: string, input: unknown, state: Block extends { state: infer S } ? S : never = 'done'): Block =>
  ({ kind: 'tool', id: name + JSON.stringify(input), name, input, state, result: null })

const t = (blocks: Block[]): Transcript => ({
  items: [{ kind: 'assistant', id: 'a', blocks }],
  draft: null, running: false, tasks: [], permissionMode: 'default', limits: null
} as unknown as Transcript)

const task = (over: Partial<TaskRun>): TaskRun => ({
  taskId: 'x', toolUseId: null, description: '実行役', subagentType: 'general',
  prompt: null, status: 'completed', summary: null, lastTool: null,
  backgrounded: false, usage: null, blocks: [], ...over
})
...
describe('触ったファイル', () => {
  ...
  it('実行役の名が無ければ説明で代える', () => {
    const out = touchedFiles(t([]), [
      task({ subagentType: null, description: '差分を読む', blocks: [tool('Read', { file_path: '/a.ts' })] })
    ])
  ...
  it('同じ実行役を二重に数えない', () => {
    const out = touchedFiles(t([]), [
      task({ subagentType: 'r', blocks: [tool('Read', { file_path: '/a.ts' }), tool('Edit', { file_path: '/a.ts' })] })
    ])
