...
const MESSAGES = [
  { type: 'system', subtype: 'init', session_id: 's1', cwd: '/Users/x/work/izuna',
    model: 'claude-opus-5', permissionMode: 'default', slash_commands: ['verify', 'code-review'] },
  ...
  { type: 'user', message: { content: [{ type: 'tool_result', tool_use_id: 't1', content: '（900 行）' }] } },
  { type: 'assistant', message: { id: 'm2', content: [
    { type: 'tool_use', id: 't2', name: 'Write',
      input: { file_path: '/Users/x/work/izuna/src/shared/skin.ts', content: 'export const a = 1\nexport const b = 2\n' } }
  ] } },
  { type: 'rate_limit_event', rate_limit_info: { unifiedWindows: { five_hour: { utilization: 0.31 }, seven_day: { utilization: 0.08 } } } }
] as never[]

const nothing = async (): Promise<never> => { throw new Error('ハーネスでは呼ばない') }
...
export function installStub(): void {
  const api: Partial<IzunaApi> = {
    ...
    listSessions: async () => [
      { id: 'aaaaaaaa-1111-2222-3333-444444444444', cwd: '/Users/x/work/izuna',
        title: 'セッションの一覧と復元', slug: 'happy-jingling-cherny',
        firstPrompt: 'CLAUDE.md を読んで', branch: 'main', cliVersion: '2.1.263',
        updatedAt: Date.parse(iso(45)), bytes: 19_000_000 },
      { id: 'bbbbbbbb-1111-2222-3333-444444444444', cwd: '/Users/x/work/izuna',
    ...
    ghPulls: async () => [],
    remotes: async () => [{ name: 'upstream', url: 'git@github.com:Watakumi/izuna.git',
      host: 'github.com', owner: 'Watakumi', repo: 'izuna', role: 'upstream' }],
